...
////////////////////////////////////////////////////////////////////////////////
void
runTest(float *input, float *output, int size)
{
	// read in signal
	unsigned int slength = size;
	float *signal = input;
	float *odata = output;


	// TODO
	// sdkReadFile(s_fname, &signal, &slength, false);
...
EXTERN_C{
	DLLTEST_API void __stdcall DwtHaar1D_Work(float *input, float *output, int size)
	{
